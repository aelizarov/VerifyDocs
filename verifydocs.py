...
def verify_npi(npi):
    try:
        ...
        if npi_data:
            ...
            if (
                record.get("enumeration_type")
                and record.get("enumeration_type") == "NPI-1"
            ):
                ...
                data["payload"]["last_name"] = record["basic"][
                    "authorized_official_last_name"
                ].lower()
                data["payload"]["first_name"] = record["basic"][
        ...
        if passport_details:
            if passport_details["valid_score"] > threshold or force_flag:
                ...
                return {
                    "message": "VALIDATED",
                    "payload": {
                        "last_name": passport_details["surname"].lower(),
                        "first_name": passport_details["names"].lower(),
                        "gender": passport_details["sex"].lower(),
                        "dob": dob,
                    },
                }
            else:
...
def validate(url, npi, dob=None, url_type=True):
    ...
    if npi_data and passport_data:
        ...
        if truth > 1 and veracity / truth > 0.5:
...
def validate(url, npi, dob=None, url_type=True):
    ...
    npi_data = valid_npi["payload"]
    passport_data = valid_passport["payload"]
    # print("Image data decode: ", passport_data)
    if npi_data and passport_data:
        veracity = 0
        truth = 0
        if npi_data.get("first_name") and passport_data.get("first_name"):
        ...
        return {
            "valid": 0,
            "payload": valid_passport["message"] + " & " + valid_npi["message"],
        }
